...
class Taiwan_Stocks(SD.Stocks_Draw):
    ...
    def Stocks_settings(self):
        ...
        if self.stock_name == "":
            ...
            if self.stock_num == "":
                ...
                ), "Please enter the stock name or number!!"
    ...
    def Check_stocks(self, df, check_name, check_num):
        if (
            df[df[check_name] == self.stock_name].empty
            and df[df[check_num] == self.stock_num].empty
        ):
            return False
            ...
            if self.stock_name != "" and self.stock_num != "":
                ...
                assert (
                    df[df[check_name] == self.stock_name][check_num].values[0] == self.stock_num
                ), "The stock name is inconsistent with the stock number!! Please enter again!!"
